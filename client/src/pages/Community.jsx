...
const Community = () => {
  ...
  const fetchImages = async () => {
    try {
      ...
      if (data.success) {
        setImages(data.images || []); // ✅ safeguard against undefined
      } else {
      ...
      {images.length > 0 ? (
        <div className="flex flex-wrap max-sm:justify-center gap-5">
          {images.map((item, index) => (
            <a
              key={item._id || index} // ✅ prefer unique key if backend provides
              href={item.imageUrl}
              target="_blank"
            ...
            >
              <img
                src={item.imageUrl}
                alt={item.userName ? `Created by ${item.userName}` : "Community image"}
