...
const Sidebar = ({isMenuOpen, setIsMenuOpen}) => {
    ...
    return (
        <div className={`flex flex-col h-screen min-w-72 p-6 dark:bg-gradient-to-b from-
        ...
        <img
            ...
            {/* New Chat Button */}
            <button className='flex justify-center items-center w-full py-1.5 mt-6 
            ...
            <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
                {
                    ...
                    includes(search.toLowerCase())).map((chat) => (
                        <div onClick={()=> {navigate('/'); setSelectedChat(chat);
                            setIsMenuOpen(false)}}
                        key={chat._id} className='p-1.5 px-4 dark:bg-[#57317C]/10 border 
            ...
            p-2 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer 
            hover:scale-103 transition-all'>
            ...
            dark:border-white/15 rounded-md cursor-pointer group'>
                ...
                {user && (<img src={assets.logout_icon} 
                    className='h-5 cursor-pointer hidden not-dark:invert group-hover:block' alt="" />  )}
            </div>
